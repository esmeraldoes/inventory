...
import ActionBillButton from '../utils/ActionButtonBill';
...
const Billings = () => {
    ...
    const loadBills = async () => {
        try {
            const data = await invoke("get_all_bills");
    ...
    const handleDeleteBill = async (billId) => {
        try {
            await invoke("delete_bill", {id: billId});
    ...
    const handlePrintChallan= (billId) => {
        navigate(`/challan/${billId}`); 
    };
    ...
    return (
      ...
      >
                        ...
                        <div style={headerBarStyle}>
                            <h3 style={titleStyle}>Billings</h3>
                    ...
                    {bills && bills.length > 0 ? (
                        bills.map((bill) => (
                            ...
                            />
                                ...
                                <td style={{ padding: '0px', textAlign: 'left' }}>{bill.id}</td>
                                <td style={{ padding: '8px', textAlign: 'left' }}>{bill.customer_name}</td>
                                <td style={{ padding: '8px', textAlign: 'center' }}>{bill.date}</td>
                                <td style={{ padding: '8px', textAlign: 'center' }}>{bill.customer_phone}</td>
                                <td style={{ padding: '8px', textAlign: 'center' }}>{bill.address}</td>
                                ...
                                onPrintBill={() => handlePrintBill(bill.id)}

                                onPrintChallan={() => handlePrintChallan(bill.id)}
                        ...
                        <tr><td colSpan="3">No Bill available</td></tr>
                    ) }
                    </tbody>
                </table>
                <div>
                <p>Selected Bills: {selectedBills.join(', ')}</p> {/* Display selected bill IDs */}
            </div>
            </div>
